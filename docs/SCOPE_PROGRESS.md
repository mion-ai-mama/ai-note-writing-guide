...
- [x] STEP1のスクリーンショット → 完了（トップページ／登録フォーム／プロフィール入力の3枚を反映）
- [x] STEP2のスクリーンショット → 完了（キーワード検索／検索結果一覧の2枚を反映）
- [ ] STEP3のスクリーンショット2点（現在は【スクリーンショット】のプレースホルダー表示）
      - ChatGPTにURLを貼り付けている画面／日本語に訳された結果の画面
- [ ] STEP4のスクリーンショット1点（現在は【スクリーンショット】のプレースホルダー表示）
      - ChatGPTにプロンプトを送った結果の画面
- [ ] OGP画像（現在はCTAバナー画像を暫定流用）
- [x] おすすめのSubstack10選 → 完了（ユーザー提供の実際の10アカウントを反映、説明文はオリジナルで書き下ろし）
